...
import { getAllRecipesForUI, getAllCategories, getAllTags } from "@/lib/types"
...
export default function HomePage() {
  ...
  const handleCategoryChange = (categorySlug: string, checked: boolean) => {
    setSelectedCategories((prev) => (checked ? [...prev, categorySlug] : prev.filter((slug) => slug !== categorySlug)))
  }

  const handleTagChange = (tagSlug: string, checked: boolean) => {
    setSelectedTags((prev) => (checked ? [...prev, tagSlug] : prev.filter((slug) => slug !== tagSlug)))
  }

  return (
    <div className="container mx-auto p-4">
      <div className="mb-6 flex flex-col items-center gap-4">
        <h1 className="text-4xl font-extrabold tracking-tight lg:text-5xl">Family Recipes</h1>
        <p className="text-xl text-muted-foreground">Delicious meals, passed down through generations.</p>
